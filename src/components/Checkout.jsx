...
const Checkout = () => {
    ...
    const { cart, cartTotal, setCart, setCartLength, setCartTotal } = useContext(CartContext)
    ...
    useEffect(() => {
      if (orderId) {
        ...
        Swal.fire({
          ...
          text: `You will be able to track your order with the following ID: ${orderId}`,
          icon: 'success',
  ...
  return (
    ...
    <div className="main">
      ...
      <form onSubmit={setOrder} className="form-container">
          <div id="exit">
            <Link to="/cart">
        ...
        <div className="heading">
          ...
          <h1 className='form-h1'>Payment details</h1>
          </div>
          <label className='form-label' htmlFor="name">Name</label>
            <input className="form-input" type="text" id="name" name="name" placeholder="Name" onChange={(e)=> setName(e.target.value)}/>
          <br/>
          <label className='form-label' htmlFor="email">Email</label>
            <input className="form-input" type="text" id="email" name="email" placeholder="Email" onChange={(e)=> setEmail(e.target.value)}/>
          <br/>
          <label className='form-label' htmlFor="card">Card number</label>
            <input className="form-input" type="text" minLength={"16"} maxLength={"16"} id="card" name="card" placeholder="0000 0000 0000 0000"/>
          <br/>
          <div className="exp-cvc">
            <div className="expiration">
          <label className='form-label' htmlFor="expiry">Expiration date</label>
            <input className="form-input" name="expiry" id="expiry" maxLength={"5"} type="text" required placeholder="00/00"/>
            <br/>
            </div>
            <div className="security">
          <label className='form-label' htmlFor="cvc">CVC</label>
            <input className="form-input" type="text" minLength={"3"} maxLength={"4"} id="cvc" name="cvc" placeholder="XXXX" />
          <br/>
            </div>
          </div>
